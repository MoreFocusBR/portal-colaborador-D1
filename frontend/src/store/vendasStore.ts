...
const useVendasStore = create<VendasState>((set, get) => ({
  ...
  fetchVendas: async () => {
    try {
      ...
      set({ 
        error: error instanceof Error ? error.message : 'Erro desconhecido ao buscar vendas', 
        loading: false 
      });
    }
  },
  ...
  alterarStatus: async (id: string, novoStatus: string) => {
    try {
      ...
      set({ 
        error: error instanceof Error ? error.message : 'Erro desconhecido ao alterar status da venda', 
        loading: false 
      });
    }
  },
