...
import useMensagensWhatsAppStore, { MensagemWhatsApp } from '../../store/mensagensWhatsAppStore';

const MensagensWhatsApp: React.FC = () => {
  const { showNotification } = useNotification();
  const { mensagens, loading, error, fetchMensagens, toggleAtivoStatus } = useMensagensWhatsAppStore();
  ...
  const colunas = [
    ...
    { 
      id: 'ativo', 
      label: 'Ativo', 
      format: (value: boolean, row: MensagemWhatsApp) => renderAtivoSwitch(value, row.id) 
    }
  ];
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Paper sx={{ p: 2, mb: 3 }}>
        <Typography variant="body1">
          Gerencie os templates de mensagens WhatsApp. As mensagens podem conter placeholders como <strong>{'{{primeiroNome}}'}</strong> ou <strong>{'{{numeroPedido}}'}</strong> que serão substituídos pelos valores reais quando a mensagem for enviada.
        </Typography>
      </Paper>
